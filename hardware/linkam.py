...
This was adapted for the HFS350EV-PB4 stage as part of a summer
...
class _GeneralStage(_LinkamBase):
    # Base class any linkam stage

    # The below was moved here from _HeatingStage for generality
    def __init__(self, uid="", **kwargs):
        # Initialise _LinkamBase class to inherit from
        super().__init__(**kwargs)
        
        # Test if we are already connected by checking name attribute - this would be the case when setting child classes
        if not hasattr(self,"name"):
            ...
            if self.init_sdk():
                ...
                if version_success:
                    ...
                    # temp_handle is a bit dodgy, but we want to retain the comms handle when we change to a specific stage class
                    global temp_handle
                    temp_handle = self._h
                    # checking connection often results in false errors.
                    #self.check_connection()
        ...
        # had problems with linkam log file
        self._process_msg(Msg.DisableLogging)

class _HeatingStage(_GeneralStage):
    ...
    def _start_heating(self, temp, rate, holdtime):
        # If one heating interval is required, call ramp() with only one interval
...
class HFS350(_HeatingStage):
    def __init__(self, uid = "", **kwargs):
        self.name = "HFS350" # must occur first, see _GeneralStage
        
        super().__init__(**kwargs)
        
        self._h = temp_handle # See _GeneralStage
...
class TS1000(_HeatingStage):
    def __init__(self, uid = "", **kwargs):
        self.name = "TS1000" # must occur first, see _GeneralStage
